...
func GetRoutes(memo Memo)[]Route{
	routes := []Route{}
...
func GetRoute(memo Memo,pos minmaxrouting.NodeIdType,i int)*Route{
	route := Route{
...
func Better(weight minmaxrouting.Weight,ws *[]*CB)bool{
	for _,w := range *ws{
		if w != nil{
			if len(weight.Weights) == 0 || len(w.Weight.Weights) == 0{
